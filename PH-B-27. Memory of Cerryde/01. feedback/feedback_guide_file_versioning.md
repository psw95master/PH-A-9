...
description: "Preferred file structure for guide docs — 드라이브는 파일 보관소이지 버전 보관소가 아니다. 버전을 남길지는 자료 유형이 정한다"
metadata: 
  node_type: memory
  type: feedback
  originSessionId: 4f44a7ed-e9a6-43fb-96a7-1c1eba62ae2f
  modified: 2026-08-14T07:20:00.000Z
---
...
>
> ⚠️ 아래 본문의 버전 규칙은 **가이드·기획 문서 유형에 한정**해서 읽을 것. 전체 규칙이 아니다.
>
> **버전을 쌓지 않는 유형 (확인된 것)**
> - **에이전트 규칙 문서** (`00. rules/STYLE.md`, `DESIGN.md`) — 매 세션 읽는 설정 파일이고, `cerry-export` 거울이 이미 최신 상태를 드라이브에 비춘다. `_v1.0` 을 따로 만들지 말 것 (260814 확정).
> - **로그 폴더가 있는 프로젝트의 마스터 가이드** — 아래 260803 예외 참조.

> ✅ **260812 확정 — 가이드 문서의 실물은 iCloud 가 아니라 구글 드라이브에 둔다.** 아래 본문의 "iCloud Drive" 는 **이 시점부로 구글 드라이브로 읽을 것** (iCloud 는 가상본만 있는 허브다).
>
> **페리의 근거** — 가이드도 결국 슬라이드로 만들 자료라 **25,000자를 넘길 일이 없다.** 드라이브 읽기 한도에 걸리지 않으므로 문서류 규칙(문서=드라이브)에 예외를 둘 이유가 없다.
>
> ⚠️ **이 결정을 떠받치는 전제는 "짧다"는 것 하나뿐이다.** 드라이브는 **끊어 읽기가 안 된다** — 한 번에 못 가져오면 뒷부분이 조용히 잘리고 되살릴 방법이 없다. 그러니:
...
- **iCloud Drive** (해당 프로젝트 폴더 — 폴더명은 개편 중이므로 기억에서 꺼내 쓰지 말고 현물을 확인할 것): each exported/synced version gets its own file with a version suffix in the filename, e.g. `PSW-C-2_세션 자동 리셋 임계치_v1.0.md`, `_v1.1.md`, `_v2.0.md` — one file per version, not one growing file with appended history. (버전 접미사 규칙은 **파일명** 규칙이라 폴더명 개편의 영향을 받지 않는다.)
